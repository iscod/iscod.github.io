...
| --- | --- | --- | --- |
| **QoS 0** | **至多一次**（at most once，发后即忘） | 低 | 可容忍偶发丢失的周期性遥测 |
| **QoS 1** | **至少一次**（at least once，需 PUBACK 确认） | 中 | **最常用**；可能重复，业务宜幂等 |
| **QoS 2** | **恰好一次**（exactly once，四步握手：PUBREC/PUBREL/PUBCOMP） | 高 | 开锁、关键指令等；成本高，需评估是否真有必要 |

> **对照**：「最多/至多一次」**只有 QoS 0**；QoS 2 是协议在该跳上**既不丢、也不重复投递**的等。
